...
(function (factory) {
    ...
    Typeahead.prototype = {
        ...
        delegateEvents: function () {
            ...
            /** to move the list down when user uses arrow keys to go up and down, and selcted item goes out of view port */
            this.node.on('keydown', function (event) { 
                    var activeElement = $('.typeahead-item.active'),
                        typeaheadParent = $('.typeahead-list'),
                        firstElement = $('.typeahead-item:first-child'),
                        getHeight = function (elements) {
                            var totalHeight = 0;
                            elements.each(function(){
                                totalHeight += $(this).innerHeight()+1;
                            });
                            return totalHeight;
                        };
                    if (event.which == 38) {
                        ...
                        if(activeElement.length == 0)
                            return;
                        if(activeElement.position().top < 0 || (activeElement.position().top + activeElement.height() > typeaheadParent.height())) {
                            var totalHeight = getHeight(activeElement.prevAll());
                            typeaheadParent.scrollTop(totalHeight);
                        }
                        ...
                        if(activeElement.length == 0)
                            return;
                        if(activeElement.position().top < 0 ||(activeElement.position().top + activeElement.height() > typeaheadParent.height())) {
                            var totalHeight = getHeight(activeElement.prevAll()) + activeElement.innerHeight();
                            var heightOfInvisiblePortion = Math.abs(firstElement.position().top);
                            typeaheadParent.scrollTop(typeaheadParent.scrollTop() +  (totalHeight - ( heightOfInvisiblePortion + typeaheadParent.height() ) ) );
                        }                       
                    }
            });
